...
spi_normalisation_m <- function(type, normdata = NULL, rawdata = NULL) {
  ...
  if (!is.null(normdata)) {stopifnot(is.data.frame(normdata))}
  if (!is.null(rawdata)) {stopifnot(is.data.frame(rawdata))}
  type <- tolower(type)
  ...
  norm_values <- rawdata[1:(which(sapply(rawdata, is.numeric))[1]-1)]
  i = 1
